...
[
  ...
  [DHashVips::IDHash, :distance, :fingerprint, 9, 22, 30, 64],
...
describe lib do
  ...
  example do |example|
    ...
    *images, bw1, bw2 = [*images, bw1, bw2].map &method(:download_and_keep)
      ...
      hashes.size.times.to_a.repeated_combination(2) do |i, j|
        ...
        when (j - i).abs == 1 && (i + j - 1) % 4 == 0
          if [i, j] == [hashes.size - 2, hashes.size - 1]
            expect(table[i][j]).to eq(described_class == DHashVips::DHash ? 4 : 0)
          else
            # STDERR.puts [table[i][j], min_similar, max_similar].inspect
